...
func getSerialNo(tablename string, row string) (id int64, err error) {
	...
	if client != nil {
		...
		var result *TResult_
		result, err = hbaseClient.Increment(context.Background(), []byte("tim_serialno"), ti)
...
func UpdateMultiple(tablename string, beans []*Bean) (er error) {
	...
	if client != nil && beans != nil && len(beans) > 0 {
		...
		er = hbaseClient.PutMultiple(context.Background(), []byte(tablename), tputs)
		if er != nil {
			panic(er.Error())
		}
	}
...
func DeleteRow(tablename string, row int64) (er error) {
	...
	if client != nil {
		...
		er = hbaseClient.DeleteSingle(context.Background(), []byte(tablename), tdelete)
		if er != nil {
			panic(er.Error())
		}
	}
...
func _DeleteRows(hbaseClient *THBaseServiceClient, tablename string, rows []int64) (err error) {
	...
	_, err = hbaseClient.DeleteMultiple(context.Background(), []byte(tablename), tdeletes)
	return
}
...
func DeleteFromQualifier(tablename string, beans []*Bean) (err error) {
	...
	if client != nil {
		...
		rs, er := hbaseClient.GetScannerResults(context.Background(), []byte(tablename), tscan, maxcount)
		if er != nil {
			panic(er.Error())
		}
		if rs != nil {
...
func _getResultByBean(hbaseClient *THBaseServiceClient, tablename string, beans []*Bean, count int32, reversed bool) (rs []*TResult_, err error) {
	...
	rs, err = hbaseClient.GetScannerResults(context.Background(), []byte(tablename), tscan, count)
	return
}
...
func ScansFromRow(tablename string, beans []*Bean, count int32, reversed bool) (results []*TResult_, err error) {
	...
	if client != nil {
		...
		if err == nil && rs != nil {
			...
			results, err = hbaseClient.GetMultiple(context.Background(), []byte(tablename), tgets)
			if err != nil {
				panic(err.Error())
			}
		}
...
func get(tablename string, row int64, family, qualifier string) (result *TResult_, err error) {
	...
	if client != nil {
		...
		result, err = hbaseClient.Get(context.Background(), []byte(tablename), tget)
		if err != nil {
			panic(err.Error())
		}
	}
