...
func (this *clientBean) Close() error {
	return this.tsock.Close()
}
...
func newClientBean() (cb *clientBean) {
	...
	for i > 0 {
		cb = new(clientBean)
		cb.tsclient, cb.tsock = _NewHbaseClient()
...
func _NewHbaseClient() (hbaseClient *THBaseServiceClient, tsocket *thrift.TSocket) {
	defer func() {
		if err := recover(); err != nil {
	...
	var er error
	tsocket, er = thrift.NewTSocketTimeout(addr, time.Duration(timeoutConns)*time.Second, time.Duration(soTimeout)*time.Second)
