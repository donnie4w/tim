...
type nodeware struct {
	tlMap        *hashmap.MapL[*tlContext, int8]
	csMap        *hashmap.Consistenthash
	mux          *lock.Numlock
	_cacheCsList []int64
	_cacheMap    *hashmap.Map[int64, int8]
}
...
func (nd *nodeware) GetUUIDNode() (_r *stub.Node) {
	_r = &stub.Node{UUID: sys.UUID, Addr: sys.Conf.CsAccess}
...
func (nd *nodeware) getRemoteNodes() (_r []*stub.RemoteNode) {
	_r = make([]*stub.RemoteNode, 0)
	nd.tlMap.Range(func(k *tlContext, _ int8) bool {
		r := &stub.RemoteNode{Addr: k.remoteAddr, UUID: k.remoteUuid, CSNUM: k.remoteCsNum, Host: k.remoteIP}
		_r = append(_r, r)
		return true
	})
	return
...
func (nd *nodeware) getcsnode(_node string) (m map[int64]int8) {
	...
	if node := nd.csnode(_node); node != sys.UUID {
		if cr, err := nd.csReqHandle(node, 0, false, &stub.CsBean{RType: 1, Bsm2: map[string][]int64{_node: nil}}); err == nil {
...
func (nd *nodeware) cswssinfo(node string) (b []byte) {
	...
	for u := range li {
		if u != sys.UUID {
			if cr, err := nd.csReqHandle(u, 0, false, &stub.CsBean{RType: 2, Bsm: map[string][]byte{node: nil}}); err == nil {
				if cr.Bsm != nil {
					b, _ = cr.Bsm[node]
				}
			}
...
/*********************************************************************/
func (nd *nodeware) csmessage(tm *stub.TimMessage, transType int8) (ok bool) {
	...
	case sys.TRANS_SOURCE:
		...
		if node := nd.csnode(toNode); node != sys.UUID {
			...
			if cr, err := nd.csReqHandle(node, 0, false, &stub.CsBean{RType: 1, Bsm2: map[string][]int64{toNode: nil}}); err == nil {
				if li, b := cr.Bsm2[toNode]; b {
					...
					for _, u := range li {
						...
						if u != sys.UUID {
							if _, err := nd.csbsHandle(u, &stub.CsBs{Bs: goutil.TEncode(tm), TransType: sys.TRANS_GOAL, BsType: sys.CB_MESSAGE}); err == nil {
								ok = true
							}
						} else if !sl {
	...
	case sys.TRANS_CONSISHASH:
		...
		if cm := getcsu(toNode); cm != nil {
			cm.Range(func(k int64, _ int8) bool {
				if k != sys.UUID {
					if _, err := nd.csbsHandle(k, &stub.CsBs{Bs: goutil.TEncode(tm), TransType: sys.TRANS_GOAL, BsType: sys.CB_MESSAGE}); err == nil {
						ok = true
					}
				} else {
					_, sl = sys.TimMessageProcessor(tm, sys.TRANS_GOAL), true
...
func (nd *nodeware) csbsMessageByCache(tm *stub.TimMessage, tonode string, nodes []int64) (rmc, ok bool) {
	...
	for _, u := range nodes {
		...
		if u != sys.UUID {
			if b, err := nd.csbsHandle(u, &stub.CsBs{Bs: goutil.TEncode(tm), Node: []byte(tonode), TransType: sys.TRANS_GOAL, BsType: sys.CB_MESSAGE, Cache: &cache}); err == nil {
				if b {
					ok = true
				} else {
					rmc = true
...
func (nd *nodeware) cspresence(tp *stub.TimPresence, transType int8) (ok bool) {
			...
			if cr, err := nd.csReqHandle(node, 0, false, &stub.CsBean{RType: 1, Bsm2: map[string][]int64{toNode: nil}}); err == nil {
				if li, b := cr.Bsm2[toNode]; b {
					...
					for _, u := range li {
						...
						if u != sys.UUID {
							if _, err := nd.csbsHandle(u, &stub.CsBs{Bs: goutil.TEncode(tp), TransType: sys.TRANS_GOAL, BsType: sys.CB_PRESENCE}); err == nil {
								ok = true
							}
						} else if !sl {
	...
	case sys.TRANS_CONSISHASH:
		...
		if cm := getcsu(toNode); cm != nil {
			cm.Range(func(k int64, _ int8) bool {
				if k != sys.UUID {
					if _, err := nd.csbsHandle(k, &stub.CsBs{Bs: goutil.TEncode(tp), TransType: sys.TRANS_GOAL, BsType: sys.CB_PRESENCE}); err == nil {
						ok = true
					}
				} else {
		...
		if tp.ToList != nil {
			if len(tp.ToList) > len(nd.csMap.Nodes())/2 && len(nd.GetRemoteUUIDS()) > 0 {
				for _, u := range tp.ToList {
					_tp := &stub.TimPresence{ID: tp.ID, FromTid: tp.FromTid, ToTid: &stub.Tid{Node: u}, SubStatus: tp.SubStatus, Offline: tp.Offline, Show: tp.Show, Status: tp.Status, Extend: tp.Extend, Extra: tp.Extra}
					sys.TimPresenceProcessor(_tp, sys.TRANS_GOAL)
				}
				for _, uuid := range nd.GetRemoteUUIDS() {
					nd.csbsHandle(uuid, &stub.CsBs{Bs: goutil.TEncode(tp), TransType: sys.TRANS_STAFF, BsType: sys.CB_PRESENCE})
				}
			} else {
				for _, u := range tp.ToList {
					_tp := &stub.TimPresence{ID: tp.ID, FromTid: tp.FromTid, ToTid: &stub.Tid{Node: u}, SubStatus: tp.SubStatus, Offline: tp.Offline, Show: tp.Show, Status: tp.Status, Extend: tp.Extend, Extra: tp.Extra}
...
func (nd *nodeware) csuser(_node string, on bool, wsId int64) (err error) {
	...
	if len(nd.getCsList()) > 1 {
		if node, ok := nd.csMap.GetStr(_node); ok && node != sys.UUID {
			err = nd.csuserHandle(node, &stub.CsUser{Node: map[string]int8{_node: 0}, Stat: stat})
		} else {
...
func (nd *nodeware) csuserBatch(uuid int64) {
	...
	if len(m) > 0 {
		nd.csuserHandle(uuid, &stub.CsUser{Node: m, Stat: 1})
	}
	vgate.VGate.Nodes().Range(func(k string, v *vgate.VRoom) bool {
		if !v.Expires() {
			if node, _ := nd.csMap.GetStr(k); node == uuid {
				vb := &stub.VBean{Rtype: 1, Vnode: k, FoundNode: &v.FoundNode}
...
func (nd *nodeware) csVbean(vb *stub.VBean) (b bool) {
...
var csuMap = hashmap.NewMapL[string, *hashmap.MapL[int64, int8]]()
...
func getcsu(node string) *hashmap.MapL[int64, int8] {
	if m, ok := csuMap.Get(node); ok {
...
func (nd *nodeware) csuserHandle(uuid int64, cu *stub.CsUser) (err error) {
	defer util.Recover()
	sys.Stat.CReqDo()
...
func (nd *nodeware) csbsHandle(uuid int64, cb *stub.CsBs) (ok bool, err error) {
	defer util.Recover()
	sys.Stat.CReqDo()
	defer sys.Stat.CReqDone()
	if uuid == sys.UUID {
...
func (nd *nodeware) csReqHandle(uuid, sendId int64, ack bool, cb *stub.CsBean) (_r *stub.CsBean, err error) {
	defer util.Recover()
	sys.Stat.CReqDo()
	defer sys.Stat.CReqDone()
	if uuid == sys.UUID {
...
func (nd *nodeware) csVrHandle(uuid, sendId int64, vb *stub.VBean) (err error) {
	defer util.Recover()
	sys.Stat.CReqDo()
	defer sys.Stat.CReqDone()
	if uuid == sys.UUID {
...
func (nd *nodeware) csuTicker() {
	...
	for {
		...
		case <-tk.C:
			func() {
				defer util.Recover()
				csuMap.Range(func(k string, cm *hashmap.MapL[int64, int8]) bool {
					mark := true
