...
func (t *TimUser) SendMBean(mbean *TimMBean) (er error) {
	...
	if CF.ConfirmAck == 1 {
		...
		er = t.Client.TimMessage(context.Background(), mbean)
		if er == nil {
			select {
			case <-timer.C:
...
func (t *TimUser) SendMBeanList(mbeans []*TimMBean) (er error) {
	...
	if CF.ConfirmAck == 1 {
		...
		er = t.Client.TimMessageList(context.Background(), mbeanList)
		select {
		case <-timer.C:
...
func (t *TimUser) Ping() (er error) {
	...
	t.Sync.Lock()
	defer t.Sync.Unlock()
	er = t.Client.TimPing(context.Background(), TimeMills())
	return
}
...
func (t *TimUser) SendPBean(pbean *TimPBean) (er error) {
	...
	t.Sync.Lock()
	defer t.Sync.Unlock()
	er = t.Client.TimPresence(context.Background(), pbean)
	return
}
...
func (t *TimUser) SendPBeanList(pbean []*TimPBean) (er error) {
	...
	er = t.Client.TimPresenceList(context.Background(), pbeanList)
	return
}
...
func (t *TimUser) SendAckBean(ackBean *TimAckBean) (er error) {
	...
	t.Sync.Lock()
	defer t.Sync.Unlock()
	er = t.Client.TimAck(context.Background(), ackBean)
	return
}
